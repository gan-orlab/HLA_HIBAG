...
#Merge allele with covariates and prs
...
for(i in 1:10){
    ...
    HLA_table <- lapply(HLA_allele_name, function(allele){
        ...
        if(FILE != "ukbb"){
            ...
            f <- formula(paste("phenotype ~ h + Z + age + sex + pc1 + pc2 + pc3 + pc4 + pc5 + pc6 + pc7 + pc8 + pc9 + pc10"))

        } else if(FILE == "ukbb"){

            f <- formula(paste("phenotype ~ h + Z + Townsend + AgeAtRecruit + sex + pc1 + pc2 + pc3 + pc4 + pc5 + pc6 + pc7 + pc8 + pc9 + pc10"))
    ...
    write_delim(result,paste0("HLA-",names(HLA)[i],"_",FILE,"_",REGION,"_prs.txt"), delim = " ", na = "NA", append = FALSE, col_names = TRUE, quote_escape = "double")
}
