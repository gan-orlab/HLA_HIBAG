...
if(FILE != "ukbb"){
    ...
    snp <- as.data.frame(fread(paste0("~/runs/eyu8/data/forest_plot/rs112485576/", FILE, ".raw")))
    names(snp)[1] <- "sample.id"
    snp[snp$rs112485576 == 2,]$rs112485576 <- 1
    ...
    snp <- as.data.frame(fread(paste0("~/runs/eyu8/data/forest_plot/rs112485576/ukbb.raw")))
    names(snp)[1] <- "sample.id"
    snp[snp$rs112485576 %in% 2,]$rs112485576 <- 1
...
#Merge allele with covariates and rs112485576
HLA <- list(A=merge(snp,merge(covar,A)),B=merge(snp,merge(covar,B)),C=merge(snp,merge(covar,C)),
    DPB1=merge(snp,merge(covar,DPB1)),DQA1=merge(snp,merge(covar,DQA1)),DQB1=merge(snp,merge(covar,DQB1)),DRB1=merge(snp,merge(covar,DRB1)),
    DRB3=merge(snp,merge(covar,DRB3)),DRB4=merge(snp,merge(covar,DRB4)),DRB5=merge(snp,merge(covar,DRB5)))

#Exclude allele where posterior probability less than 0.5, and samples with 2 alleles with low posterior probability
...
for(i in 1:10){
    ...
    HLA_table <- lapply(HLA_allele_name, function(allele){
        ...
        if(FILE != "ukbb"){
            ...
            f <- formula(paste("phenotype ~ h + rs112485576 + age + sex + pc1 + pc2 + pc3 + pc4 + pc5 + pc6 + pc7 + pc8 + pc9 + pc10"))

        } else if(FILE == "ukbb"){

            f <- formula(paste("phenotype ~ h + rs112485576 + Townsend + AgeAtRecruit + sex + pc1 + pc2 + pc3 + pc4 + pc5 + pc6 + pc7 + pc8 + pc9 + pc10"))
    ...
    write_delim(result,paste0("HLA-",names(HLA)[i],"_",FILE,"_",REGION,"_rs112485576.txt"), delim = " ", na = "NA", append = FALSE, col_names = TRUE, quote_escape = "double")
}
