   ...
   filtered_HLA_gene <- filtered_HLA[[i]][filtered_HLA[[i]]$prob > 0.5,]
    ...
    HLA_table <- lapply(HLA_allele_name, function(allele){
        ...
        if(FILE != "ukbb"){
            ...
            f <- formula(paste("phenotype ~ h + DRB1_Pos13H + age + sex + pc1 + pc2 + pc3 + pc4 + pc5 + pc6 + pc7 + pc8 + pc9 + pc10"))

        } else if(FILE == "ukbb"){

            f <- formula(paste("phenotype ~ h + DRB1_Pos13H + Townsend + AgeAtRecruit + sex + pc1 + pc2 + pc3 + pc4 + pc5 + pc6 + pc7 + pc8 + pc9 + pc10"))
    ...
    write_delim(result,paste0("HLA-",names(HLA)[i],"_",FILE,"_",REGION,"_Pos13H.txt"), delim = " ", na = "NA", append = FALSE, col_names = TRUE, quote_escape = "double")
}
