...
DQA1_0303 <- HLA_DQA1_count[,c("sample.id","03:03")]
if(any(DQA1_0303$"03:03" == 2)){
    DQA1_0303[DQA1_0303$"03:03" == 2,]$"03:03" <- 1
}
names(DQA1_0303) <- c("sample.id","DQA1_0303")
...
for(i in 1:10){
    ...
    if(i == 5){
        HLA_allele_name <- names(HLA_count)[names(HLA_count) != "03:03"]
    }
    ...
    HLA_table <- lapply(HLA_allele_name, function(allele){
        ...
        HLA_glm_adj <- merge(HLA_glm,DQA1_0303)
        HLA_glm_adj$DQA1_0303 <- as.numeric(HLA_glm_adj$DQA1_0303)

        if(FILE != "ukbb"){

            f <- formula(paste("phenotype ~ h + DQA1_0303 + age + sex + pc1 + pc2 + pc3 + pc4 + pc5 + pc6 + pc7 + pc8 + pc9 + pc10"))

        } else if(FILE == "ukbb"){

            f <- formula(paste("phenotype ~ h + DQA1_0303 + Townsend + AgeAtRecruit + sex + pc1 + pc2 + pc3 + pc4 + pc5 + pc6 + pc7 + pc8 + pc9 + pc10"))
    ...
    write_delim(result,paste0("HLA-",names(HLA)[i],"_",FILE,"_",REGION,"_DQA1_0303.txt"), delim = " ", na = "NA", append = FALSE, col_names = TRUE, quote_escape = "double")
}
