...
aa <- aa[,c(1,6:9,13)]
names(aa) <- c("Allele","V1000","V2000","V3000","V4000","V5000")

result <- merge(merge(merge(merge(merge(hla, rs, by = "Allele", all.x = T ), dqa1_0301, by = "Allele", all.x = T ),
	dqa1_0303, by = "Allele", all.x = T ), prs, by = "Allele", all.x = T ), aa, by = "Allele", all.x = T )
...
write.xlsx(result,paste0(args[7]))
