...
for(i in 7:7){
    ...
    hla_aa_result <- lapply(hla_pos, function(pos){
            ...
            HLA_table <- lapply(HLA_allele_name, function(allele){
                ...
                HLA_allele_freq <-  table(HLA_glm[, c(allele, "phenotype")])
                if(nrow(HLA_allele_freq)>2){
                    HLA_freq <- signif((HLA_allele_freq[2,1] + HLA_allele_freq[2,2] + 2*(HLA_allele_freq[3,1] + HLA_allele_freq[3,2]))/sum(HLA_allele_freq*2),3)
                    HLA_allele_freq_case <- signif((HLA_allele_freq[2,2]+2*HLA_allele_freq[3,2])/(2*(HLA_allele_freq[2,2]+HLA_allele_freq[1,2]+HLA_allele_freq[3,2])),3)
                    HLA_allele_freq_control <- signif((HLA_allele_freq[2,1]+2*HLA_allele_freq[3,1])/(2*(HLA_allele_freq[2,1]+HLA_allele_freq[1,1]+HLA_allele_freq[3,1])),3)
                    HLA_ncase <- HLA_allele_freq[1,2] + HLA_allele_freq[2,2] + HLA_allele_freq[3,2]
                    HLA_ncontrol <- HLA_allele_freq[1,1] + HLA_allele_freq[2,1] + HLA_allele_freq[3,1]
                    HLA_ntotal <- sum(HLA_allele_freq)

                } else{
                    HLA_freq <- signif((HLA_allele_freq[2,1] + HLA_allele_freq[2,2])/sum(HLA_allele_freq),3)
                    HLA_allele_freq_case <- signif(HLA_allele_freq[2,2]/(2*(HLA_allele_freq[1,2] + HLA_allele_freq[2,2])),3)
                    HLA_allele_freq_control <- signif(HLA_allele_freq[2,1]/(2*(HLA_allele_freq[1,1] + HLA_allele_freq[2,1])),3)
                    HLA_ncase <- HLA_allele_freq[1,2] + HLA_allele_freq[2,2]
                    HLA_ncontrol <- HLA_allele_freq[1,1] + HLA_allele_freq[2,1]
                    HLA_ntotal <- sum(HLA_allele_freq)
                }
            ...
            if(!grepl(allele,row.names(var)[2])){
                ...
                return(cbind(Pos = paste0("Pos",pos + increment,allele), Ref = "z", Alt = allele, HLA_ntotal, HLA_ncase, HLA_ncontrol, HLA_freq, HLA_allele_freq_case, HLA_allele_freq_control, Reduce(cbind, var_list)))})
            result <- Reduce(rbind, HLA_table)
            return(result)
            })

    hla_result <- Reduce(rbind, hla_aa_result)
    write_delim(hla_result,paste0("HLA-",names(HLA)[i],"_",FILE,"_",REGION,"_aa_additive.txt"), delim = " ", na = "NA", append = FALSE, col_names = TRUE, quote_escape = "double")
}
